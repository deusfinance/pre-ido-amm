...
contract PreIDOToken is ReentrancyGuard, ERC20, Ownable {
    ...
    mapping(address => uint) public exchangedAmount;
    /* ========== EVENTS ========== */
    ...
    event ExchangeBoughtAmount(address user, uint amount);

    /* ========== CONSTRUCTOR ========== */

    constructor(
        address[] memory addrs,  // addr[0]: _creator_address, addr[1]: _collateralAddress, addr[2]: _powerLibrary, addr[3]: deusGovernanceAddress, addr[4]: muonContract
        uint32 fee_,
    ...
    function exchangeBoughtAmount(
        uint exchangableAmount,
        uint amount,
        bytes calldata reqId,
        SchnorrSign[] calldata sigs
    ) external nonReentrant {
        require(amount + exchangedAmount[msg.sender] <= exchangableAmount, "PreIDOToken: your amount is extra than bought amount");
        require(sigs.length >= 1, "Bridge: insufficient number of signatures");
        {
            bytes32 hash = keccak256(
            abi.encodePacked(
                abi.encodePacked(exchangableAmount),
                abi.encodePacked(msg.sender, ETH_APP_ID)
                )
            );

            IMuonV02 muon = IMuonV02(muonContract);
            require(muon.verify(reqId, uint(hash), sigs), "Bridge: not verified");
        }
        _mint(msg.sender, amount);
        emit ExchangeBoughtAmount(msg.sender, amount);
    }
    ...
    function migrateStep2() external onlyDeusGovernance {
        ...
        IERC20(realTokenAddress).transferFrom(partnerAddress, address(this), totalSupply() * claimRatio / scale);
